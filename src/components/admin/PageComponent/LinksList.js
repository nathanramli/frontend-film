...
class  LinksList  extends  Component {
  ...
  handleUpdate(){
    ...
    let url = `${API_URL}/api/link_detail/${this.state.pk}`;
    axios.put(url, form_data, {
      ...
      let pk = this.state.pk;
        ...
        this.setState({links:  [...newArr, result.data]});
  ...
  handleEdit(e, pk){
    let data =  this.state.links.filter(function(obj) {
        return  obj.pk === pk;
    });
      ...
      this.setState({
        open: true,
        pk: pk,
        episode: data[0].judul_link,
        url1080p: data[0].url1080p[0] !== "" ? data[0].url1080p[0].split(',') : [],
        url720p: data[0].url720p[0] !== "" ? data[0].url720p[0].split(',') : [],
        url540p: data[0].url540p[0] !== "" ? data[0].url540p[0].split(',') : [],
        url480p: data[0].url480p[0] !== "" ? data[0].url480p[0].split(',') : [],
        url360p: data[0].url360p[0] !== "" ? data[0].url360p[0].split(',') : [],
        url240p: data[0].url240p[0] !== "" ? data[0].url240p[0].split(',') : []
      });
  }
    ...
    return (
        <Container>
                  ...
                  <TableBody>
                    {this.state.links.map(row =>
                      <TableRow key={row.pk}>
                        ...
                        <TableCell>
                        <Button size="small" color="secondary" variant="contained" onClick={(e) => {if(window.confirm('Anda yakin ingin menghapus link download anime ini?')) this.hapusEpisode(e, row.pk)}}><DeleteIcon/></Button>&nbsp;
                        <Button size="small" color="primary" variant="contained" onClick={(e) => this.handleEdit(e, row.pk)}><EditIcon/></Button>
                        </TableCell>
                      </TableRow>
                    )}
                  </TableBody>
          ...
          <Dialog open={this.state.open} onClose={this.toggleClose.bind(this)} fullWidth maxWidth="lg" aria-labelledby="title">
            ...
            <DialogTitle id="title">{this.state.pk === '' ? 'Tambah' : 'Edit'} Link Download</DialogTitle>
            <DialogContent dividers>
                <TextField onChange={this.handleChangeEpisode.bind(this)} value={this.state.episode} label="Judul Episode" helperText="Gunakan tagar(#) untuk mengurutkan misal: #01#Episode 1." margin="normal" fullWidth required/>
                <Table>
                  <TableHead>
                    <TableRow>
                      <TableCell>
                      ...
                      <TableCell colSpan={2}>
                        <Fab size="small" color="primary" onClick={(e) => this.addInput(e)}>+</Fab>&nbsp;&nbsp;
                        <FormControl>
                            ...
                            return (
                              <TableRow>
                                <TableCell colSpan={2} width="45%">
                                  <TextField style={{width: '100%'}} onChange={(e) => this.handleInput1080JudulChange(e, index)} placeholder="Judul link. contoh: Google Drive" value={isi[0]} required/>
                                </TableCell>
                                <TableCell colSpan={2} width="45%">
                                  <TextField style={{width: '100%'}} onChange={(e) => this.handleInput1080LinkChange(e, index)} placeholder="Link. contoh: http://google.com/" value={isi[1]} required/>
                                </TableCell>
                                <TableCell width="10%">
                      ...
                      {this.state.url720p.map((link, index) => {
                        const isi = link.split('|||');
                          return (
                              <TableRow>
                                <TableCell colSpan={2} width="45%">
                                      <TextField style={{width: '100%'}} onChange={(e) => this.handleInput720JudulChange(e, index)} placeholder="Judul link. contoh: Google Drive" value={isi[0]} required/>
                                </TableCell>
                                <TableCell colSpan={2} width="45%">
                                      <TextField style={{width: '100%'}} onChange={(e) => this.handleInput720LinkChange(e, index)} placeholder="Link. contoh: http://google.com/" value={isi[1]} required/>
                                </TableCell>
                                <TableCell width="10%">
                      ...
                      {this.state.url540p.map((link, index) => {
                        const isi = link.split('|||');
                          return (
                              <TableRow>
                                <TableCell colSpan={2} width="45%">
                                  <TextField style={{width: '100%'}} onChange={(e) => this.handleInput540JudulChange(e, index)} placeholder="Judul link. contoh: Google Drive" value={isi[0]} required/>
                                </TableCell>
                                <TableCell colSpan={2} width="45%">
                                <TextField style={{width: '100%'}} onChange={(e) => this.handleInput540LinkChange(e, index)} placeholder="Link. contoh: http://google.com/" value={isi[1]} required/>
                                </TableCell>
                                <TableCell width="10%">
                      ...
                      {this.state.url480p.map((link, index) => {
                        const isi = link.split('|||');
                          return (
                              <TableRow>
                                <TableCell colSpan={2} width="45%">
                                      <TextField style={{width: '100%'}} onChange={(e) => this.handleInput480JudulChange(e, index)} placeholder="Judul link. contoh: Google Drive" value={isi[0]} required/>
                                </TableCell>
                                <TableCell colSpan={2} width="45%">
                                      <TextField style={{width: '100%'}} onChange={(e) => this.handleInput480LinkChange(e, index)} placeholder="Link. contoh: http://google.com/" value={isi[1]} required/>
                                </TableCell>
                                <TableCell width="10%">
                      ...
                      {this.state.url360p.map((link, index) => {
                        const isi = link.split('|||');
                          return (
                              <TableRow>
                                <TableCell colSpan={2} width="45%">
                                      <TextField style={{width: '100%'}} onChange={(e) => this.handleInput360JudulChange(e, index)} placeholder="Judul link. contoh: Google Drive" value={isi[0]} required/>
                                </TableCell>
                                <TableCell colSpan={2} width="45%">
                                      <TextField style={{width: '100%'}} onChange={(e) => this.handleInput360LinkChange(e, index)} placeholder="Link. contoh: http://google.com/" value={isi[1]} required/>
                                </TableCell>
                                <TableCell width="10%">
                      ...
                      {this.state.url240p.map((link, index) => {
                        const isi = link.split('|||');
                          return (
                              <TableRow>
                                <TableCell colSpan={2} width="45%">
                                      <TextField style={{width: '100%'}} onChange={(e) => this.handleInput240JudulChange(e, index)} placeholder="Judul link. contoh: Google Drive" value={isi[0]} required/>
                                </TableCell>
                                <TableCell colSpan={2} width="45%">
                                      <TextField style={{width: '100%'}} onChange={(e) => this.handleInput240LinkChange(e, index)} placeholder="Link. contoh: http://google.com/" value={isi[1]} required/>
                                </TableCell>
                                <TableCell width="10%">
            ...
            <DialogActions>
                <Button type="submit" variant="contained" color="primary">{this.state.pk === '' ? 'Tambah' : 'Edit'}</Button>
            </DialogActions>
            </form>
          </Dialog>
        </Container>
